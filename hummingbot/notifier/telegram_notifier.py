...
class TelegramNotifier(NotifierBase):
    ...
    async def send_msg_async(self, msg: str, bot: Bot = None) -> None:
        ...
        try:
            try:
                await self._async_call_scheduler.call_async(
                    lambda: bot.send_message(
                        self._chat_id, text=formatted_msg, parse_mode=ParseMode.HTML, reply_markup=reply_markup
                    )
                )
                ...
                self.logger().network(
                    f"Telegram NetworkError: {network_err.message}! Trying one more time", exc_info=True
                )
                await self._async_call_scheduler.call_async(
                    lambda: bot.send_message(
                        self._chat_id, text=msg, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup
                    )
                )
        except TelegramError as telegram_err:
            self.logger().network(f"TelegramError: {telegram_err.message}! Giving up on that message.", exc_info=True)
