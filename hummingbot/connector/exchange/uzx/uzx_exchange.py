...
class UzxExchange(ExchangePyBase):
    ...
    async def track_all_open_orders(self, market: str):
        ...
        try:
            ...
            self.logger().error(f"Error in track_all_open_orders (get_unfilled_response): {type(e).__name__}: {e}")
            raise

        for order in unfilled_or_partially_filled_response:
            try:
                if any(
                    [str(ifo.exchange_order_id) == str(order["order_id"]) for ifo in self.in_flight_orders.values()]
                ):
                    continue
