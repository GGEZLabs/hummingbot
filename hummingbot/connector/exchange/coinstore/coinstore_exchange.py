...
class CoinstoreExchange(ExchangePyBase):
    ...
    def _cleanup_caches(self):
        ...
        expired_order_info_keys = [
            key
            for key, value in self._order_info_cache.items()
        ...
        expired_unfilled_keys = [
            key
            for key, value in self._unfilled_or_partially_filled_responses_cache.items()
        ...
        stale_fill_keys = [key for key in self._processed_trade_fills.keys() if key not in active_order_ids]
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        try:
            ...
            if found_in_open_orders:
                ...
                fill_price = (
                    (cumulative_fill_amt / cumulative_fill_qty) if cumulative_fill_qty > 0 else open_order_price
                )
