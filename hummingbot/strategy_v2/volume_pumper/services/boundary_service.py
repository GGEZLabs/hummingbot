...
class BoundaryService:
    ...
    def _generate_level_orders(
        ...
        for level in levels:
            try:
                ...
                min_amount = round_to_tick_size(min_notional / price, self.amount_tick_size)
    ...
    def is_order_book_valid(self, config: VolumePumperMarketConfig) -> bool:
        ...
        Valid means:
        - We have orders at the flexible support level (unless at static boundary)
        - We have orders at the flexible resistance level (unless at static boundary)

        When flexible equals static boundary, no order is required at that level
        since we're already at the edge of the allowed range.
        ...
        """
        has_support_order = config.support_at_static or self._order_adapter.has_order_at_price(
            config.flexible_support,
            tolerance_ticks=10,
            tick_size=self.price_tick_size,
        )

        has_resistance_order = config.resistance_at_static or self._order_adapter.has_order_at_price(
    ...
    def detect_conflicting_orders(
        ...
        try:
            ...
            self._market_data.connector.logger().error(f"Error detecting conflicting orders: {type(e).__name__}: {e}")
    ...
    def _subtract_my_orders_from_orderbook(
        ...
        if not orderbook_df.empty:
            print(f"Remaining orderbook: {orderbook_df}")
        return orderbook_df
    ...
    def _filter_orderbook_to_range(
        ...
        bids_in_range = bids[bids["price"].apply(lambda p: compare_numbers(p, ">", config.flexible_support))].copy()

        # Filter asks: keep only those < flexible_resistance
        asks_in_range = asks[asks["price"].apply(lambda p: compare_numbers(p, "<", config.flexible_resistance))].copy()
