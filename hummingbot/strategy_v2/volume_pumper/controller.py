...
class VolumePumperController(ControllerBase):
    ...
    async def _refresh_order_book_task(self) -> None:
        ...
        try:
            ...
            optimized_plan, conflicts = self._boundary_service.check_action_plan_conflicts(
                plan, self._market_config
            )
            ...
            if conflicts:
                ...
                self.logger().notify(
                    f"Detected {len(conflicts)} conflicting orders from other traders. "
                    f"Entering cooldown mode."
                )
            else:
